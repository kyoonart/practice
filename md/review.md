...
- csp（内容安全策略）本质上建立白名单，明确告诉了浏览器那些外部资源可以加载和执行 例如 只允许加载本站资源 只允许加载 http 协议图片 不允许加载任何来源的框架
  ...
  \*\* token 验证原理：我们一般把 token 存在 cookie 中然后发送请求时带上自定义 csrf-token header 这个值就是 token 值校验原理就是验证 cookie 中的 token 和 header 中的 token 是否一致 ，第三方请求及时可以带上 cookie 但是没有自定义 header 所以不能通过验证（这个前提是排除了 xss 攻击和 cookie 没有设置 httponly 属性
...
Code Splitting：将代码按路由维度或者组件分块（chunks），这样做到按需加载，同时可以充分利用浏览器缓存
scope-hosting :
